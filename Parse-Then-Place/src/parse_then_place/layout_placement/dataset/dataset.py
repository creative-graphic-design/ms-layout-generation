...
from parse_then_place.layout_placement.placement_utils.utils import read_json, read_txt
...
class PlacementDataset(Dataset):
    ...
    def _process_stage_one_prediction(self, predictions):
        grammar_dir = Path(__file__).resolve().parents[2] / "ir"
        if self.config.args.dataset_name == 'web':
            executor = Web_Executor(str(grammar_dir / "grammar_web.lark"))
        elif self.config.args.dataset_name == 'rico':
            executor = Rico_Executor(str(grammar_dir / "grammar_rico.lark"))
        self.source_data = []
        self.target_data = []
