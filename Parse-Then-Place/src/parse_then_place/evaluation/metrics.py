...
from parse_then_place.evaluation.layout_net import LayoutNet
from parse_then_place.layout_placement.placement_utils.utils import CANVAS_SIZE, LABEL, LABEL2ID
...
def create_fid_model(dataset, device='cpu'):
    fid_net_dir = Path(__file__).resolve().parent / "net"
    if dataset == 'web':
        fid_net_path = str(fid_net_dir / "fid_web_finetune.pth.tar")
