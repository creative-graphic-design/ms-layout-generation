...
def gaussian_matrix2(t, bt):
    ...
    values = np.linspace(
        start=0.0, stop=127.0, num=num_pixel_vals, endpoint=True, dtype=np.float64
    )
    ...
    for k in range(1, transition_bands + 1):
        off_diag = np.full(
            shape=(num_pixel_vals - k,), fill_value=values[k], dtype=np.float64
        )
...
def alpha_schedule(
    ...
    if matrix_policy == 1:  # for gaussian refine
        ...
        att = np.concatenate(
            (
                np.arange(0, time_step * sep_1 // sep)
                / (time_step * sep_1 // sep - 1)
                * (0.0001 - 0.99999)
                + 0.99999,
                np.arange(0, time_step - time_step * sep_1 // sep)
                / (time_step - time_step * sep_1 // sep - 1)
                * (0.000009 - 0.00009)
                + 0.00009,
            )
        )
        ...
        bt2 = np.concatenate(
            (bt2[: time_step * sep_1 // sep], at1[time_step * sep_1 // sep :] / 128)
        )
        at = np.concatenate(
            (
                at[: time_step * sep_1 // sep],
                (1 - ct - bt2 * 128)[time_step * sep_1 // sep :],
            )
        ).clip(min=1e-30)
        ct = np.concatenate(
            ((1 - at - bt2)[: time_step * sep_1 // sep], ct[time_step * sep_1 // sep :])
        ).clip(min=1e-30)
    ...
    if (
        matrix_policy == 2
    ):  # gaussian refine (wo bbox absorb) produce similar results as policy=1
        sep = 5
        sep_1 = sep - 1

        att = np.concatenate(
            (
                np.arange(0, time_step * 4 // 5)
                / (time_step * 4 // 5 - 1)
                * (0.0001 - 0.99999)
                + 0.99999,
                np.arange(0, time_step - time_step * 4 // 5)
                / (time_step - time_step * 4 // 5 - 1)
                * (0.99999 - 0.0001)
                + 0.0001,
            )
        )
        ...
        ctt = (
            np.arange(0, time_step) / (time_step - 1) * (0.00009 - 0.000009) + 0.000009
        )
        ctt = np.concatenate(([0], ctt))
        one_minus_ctt = 1 - ctt
        one_minus_ct = one_minus_ctt[1:] / one_minus_ctt[:-1]
...
class DiffusionTransformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        ):  # in case refine case overwrite replace schedule
            self.gaussian_matrix = True  # for gaussian

        if isinstance(matrix_policy, tuple):
        ...
        self.type_classes = (
            num_classes - 1 - 128 - 5
        )  # -1 is the mask token; -128 is the coord token; -5 is the special token
        ...
        if self.gaussian_matrix:  # gaussian refine
            at, at1, bt1, bt2, ct, ct1, att, att1, btt1, btt2, ctt, ctt1 = (
                alpha_schedule(
                    self.num_timesteps, N=self.num_classes - 1, matrix_policy=1
                )
            )
            ...
            at, at1, bt1, bt2, ct, ct1, att, att1, btt1, btt2, ctt, ctt1 = (
                alpha_schedule(
                    self.num_timesteps, N=self.num_classes - 1, matrix_policy=2
                )
            )
        ...
        assert log_add_exp(log_ct1, log_1_min_ct1).abs().sum().item() < 1.0e-5
        assert (
            log_add_exp(log_cumprod_ct1, log_1_min_cumprod_ct1).abs().sum().item()
            < 1.0e-5
        )
        ...
        bt2 = torch.where(bt2 == 0.0, bt2.max(), bt2)
        q_one_step_mats = [
            gaussian_matrix2(t, bt=bt2.pow(2).pow(pow_num / 2) * mul_num)
        ...
        assert self.q_mats.shape == (self.num_timesteps + 1, 128, 128), (
            self.q_mats.shape
        )
        ...
        if self.gaussian_matrix:  # for gaussian_relu
            ...
            matrix1 = torch.cat(
                [
                    torch.cat(
                        [
                            torch.eye(5, device=device).expand(bz, -1, -1),
                            torch.zeros(bz, 5, self.num_classes - 5, device=device),
                        ],
                        dim=-1,
                    ),
                    torch.cat(
                        [
                            ...
                            torch.zeros(
                                bz,
                                self.type_classes,
                                self.num_classes - 5 - self.type_classes,
                                device=device,
                            ),
                        ],
                        dim=-1,
                    ...
                    torch.cat(
                        [
                            ...
                            torch.zeros(
                                bz,
                                128,
                                self.num_classes - 5 - self.type_classes - 128,
                                device=device,
                            ),
                        ],
                        dim=-1,
                    ...
                    torch.cat(
                        [
                            torch.zeros(bz, 1, 5, device=device),
                            log_add_exp(
                                torch.zeros(bz, 1, self.type_classes, device=device)
                                .clamp(min=1e-30)
                                .log()
                                + log_1_min_ct1,
                                log_ct1,
                            ...
                            log_add_exp(
                                torch.zeros(bz, 1, 128, device=device)
                                .clamp(min=1e-30)
                                .log()
                                + log_1_min_ct,
                                log_ct,
                            ).exp(),
                            torch.ones(bz, 1, 1, device=device),
                        ],
                        dim=-1,
                    ),
                ],
            ...
            matrix2 = torch.cat(
                [
                    ...
                    torch.cat(
                        [
                            torch.zeros(bz, 128, self.type_classes + 5, device=device),
                            self.q_onestep_mats.to(device)[t],
                            torch.zeros(
                                bz,
                                128,
                                self.num_classes - 5 - self.type_classes - 128,
    ...
    def q_pred(self, log_x_start, t):  # q(xt|x0)
        ...
        if self.gaussian_matrix:
            ...
            log_1_min_cumprod_ct1 = extract(
                self.log_1_min_cumprod_ct1, t, (1, 1, 1)
            )  # 1-ct~
    ...
    def q_posterior(
        ...
        if self.schedule_type == "relu":
            ...
            ct_cumprod_vector = torch.cat(
                [
                    log_zero_vector_aux.expand(-1, 5, -1),
                    log_cumprod_ct1.expand(-1, self.type_classes, -1),
                    log_cumprod_ct.expand(
                        -1, self.num_classes - 1 - self.type_classes - 5, -1
                    ),
                ],
                dim=1,
            )
            ...
            ct_cumprod_vector = torch.cat(
                [
                    log_zero_vector_aux.expand(-1, 5, -1),
                    log_cumprod_ct.expand(-1, self.num_classes - 1 - 5, -1),
                ],
                dim=1,
            )
        ...
        if self.schedule_type == "relu":
            ...
            ct_vector = torch.cat(
                [
                    log_zero_vector_aux.expand(-1, 5, -1),
                    log_ct1.expand(-1, self.type_classes, -1),
                    log_ct.expand(-1, self.num_classes - 1 - self.type_classes - 5, -1),
                ],
                dim=1,
            )
        else:
            ct_vector = torch.cat(
                [
                    log_zero_vector_aux.expand(-1, 5, -1),
                    log_ct.expand(-1, self.num_classes - 1 - 5, -1),
                ],
                dim=1,
            )
    ...
    def p_pred(
        self, log_x, model, t, y=None
    ):  # if x0, first p(x0|xt), than sum(q(xt-1|xt,x0)*p(x0|xt))
        if self.parametrization == "x0":
            log_x_recon = self.cf_predict_start(log_x, model, y, t)
    ...
    @torch.no_grad()
    def p_sample(
        self, log_x, model, t, sampled=None, to_sample=None, y=None
    ):  # sample q(xt-1) for next step from  xt, actually is p(xt-1|xt)
        model_log_prob, log_x_recon = self.p_pred(log_x, model, t, y=y)
    ...
    def training_losses(self, model, x, model_kwargs=None, is_train=True):
        ...
        log_x0_recon = self.predict_start(
            log_xt, model, y=model_kwargs["input_ids"], t=t
        )
        ...
        for index in range(t.size()[0]):
            ...
            ).sum().cpu() / x0_real.size()[1]  # the ratio of same codebook
        ...
        Lt2 = kl_loss.pow(2)
        device = t.device
        Lt2_prev = self.Lt_history.to(device).gather(dim=0, index=t)
        new_Lt_history = (0.1 * Lt2 + 0.9 * Lt2_prev).detach()
        self.Lt_history.to(device).scatter_(dim=0, index=t, src=new_Lt_history)
        self.Lt_count.to(device).scatter_add_(dim=0, index=t, src=torch.ones_like(Lt2))
        ...
        **kwargs,
        ...
        if not gen_refine:
            ...
            if y is None or constrained == "completion":
                ...
                type_mask = (
                    torch.tensor([0, 1, 0, 0, 0, 0] * 20 + [0]).to(device).bool()
                )
                padding_mask = (torch.masked_select(y, type_mask) != 3).reshape(
                    batch_size, -1
                )
            ...
            input = (
                torch.ones(
                    batch_size, self.content_seq_len, dtype=torch.int64, device=device
                )
                * 3
            )  ##padding
            ...
            if constrained == "type":
                ...
                noise = (
                    m.sample(torch.tensor([batch_size, self.content_seq_len])).to(
                        device
                    )
                    + self.type_classes
                    + 5
            ...
            if self.ori_schedule_type.startswith(
                "gaussian_refine_pow2.5_wo_bbox_absorb"
            ):  # wo bbox absorb
        ...
        with torch.no_grad():
            ...
            for diffusion_index in tqdm(diffusion_list):
                ...
                if multistep and (
                    diffusion_index
                    in np.linspace(0, start_step - 1, num=20).astype(int)
                ):
