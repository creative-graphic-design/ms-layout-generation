            ...
            if data_args.training_mode == "bit":
                ...
                def int2bit(x, n=7):
                    ...
                    x = torch.bitwise_right_shift(
                        x.unsqueeze(-1).int(),
                        torch.arange(0, n, dtype=torch.int64, device=device),
                    )
...
def get_corpus_rocstory(
    data_args, model, seq_length, padding_mode="block", split="train", load_vocab=None
):
    import torch
    ...
    if model is None and data_args.experiment == "random":
        ...
        if os.path.exists(path_save):
            model.load_state_dict(torch.load(path_save, map_location="cpu"))
        else:
    ...
    if data_args.experiment_mode == "lm":
        result_train_lst = helper_tokenize_encode(
            sentence_lst, vocab_dict, model, seq_length, data_args, padding_mode
        )
    elif data_args.experiment_mode == "conditional_gen":
        result_train_lst = helper_tokenize_encode_cond(
            sentence_lst, vocab_dict, model, seq_length, data_args
        )
