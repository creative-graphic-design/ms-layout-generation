...
class GaussianDiffusion:
    ...
    def p_sample_loop_progressive(
        ...
        progress=False,
        top_p=None,
        langevin_fn=None,  # align with ddim
        eta=None,
    ):
        """
        Generate samples from the model and yield intermediate samples from
        ...
        if noise is not None:
            img = noise
            indices = list(range(self.num_timesteps // 2))[::-1]
        else:
            img = th.randn(*shape, device=device)
    ...
    def x0_helper(self, model_output, x, t):
        if self.model_mean_type == ModelMeanType.PREVIOUS_X:
            ...
            if self.model_mean_type == ModelMeanType.START_X:
                pred_xstart = model_output  # here just return model_output
            else:
    ...
    def training_losses_e2e(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        rico = 0
        pad_mask = None
        input_ids = model_kwargs.pop("input_ids").to(
            t.device
        )  # for layout this is type

        if "pad_mask" in model_kwargs:
            pad_mask = model_kwargs.pop("pad_mask").to(t.device)

        if x_start.shape[1] == 20:
            rico = 1
            x_start_mean = x_start
        else:
            x_start_mean = model.model.module.get_embeds(input_ids)
        if self.ungen:  # uncon generation maybe something wrong
            x_start_mean = th.cat(
                [x_start[:, :, :4], model.model.module.get_embeds(input_ids)], -1
            )
            ...
            if self.self_cond:
                if random() < 0.5:
                    with th.no_grad():
                        ...
                        if rico and pad_mask is not None:
                            self_cond = model(
                                x_t,
                            ...
                            self_cond = self_cond.detach()
                        elif rico and pad_mask is None:
                            self_cond = model(
                                x_t, self._scale_timesteps(t), y=input_ids
                            )
                            self_cond = self_cond.detach()

            if not rico:
                model_output = model(
                    x_t, self._scale_timesteps(t), self_cond=self_cond, **model_kwargs
                )
            if rico and pad_mask is not None:
                model_output = model(
                    x_t,
                ...
                model_output = model(
                    x_t, self._scale_timesteps(t), self_cond=self_cond, y=input_ids
                )
            if self.model_var_type in [
                ModelVarType.LEARNED,
                ModelVarType.LEARNED_RANGE,
    ...
    def training_losses_e2e_simple(
        ...
        if self.loss_type == LossType.E2E_Simple_KL:
            ...
            ce_loss = ce(
                model_logits.view(-1, model_logits.size(-1)), input_ids.view(-1)
            )
    ...
    def calc_bpd_loop_e2e(
        ...
        # if self.ungen:
        #     std=th.zeros_like(x_start_mean).to(x_start_mean.device)
