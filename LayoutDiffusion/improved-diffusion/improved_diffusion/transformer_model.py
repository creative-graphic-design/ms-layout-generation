...
class TransformerModel(nn.Module):
    ...
    def forward(
        self, x, timesteps, self_cond=None, y=None, src_ids=None, src_mask=None
    ):
        """
        ...
        temb = timestep_embedding(timesteps, self.model_channels).type(x.dtype)
...
class DiscreteTransformerModel(nn.Module):
    ...
    def __init__(
        ...
        self.input_up_proj = nn.Sequential(
            nn.Linear(self.in_channels, config.hidden_size),
            nn.Tanh(),
            nn.Linear(config.hidden_size, config.hidden_size),
        )
        if init_pretrained:
    ...
    def forward(self, x, timesteps, y=None):
        ...
        if self.constrained == "type":
            ...
            mask2 = (
                torch.tensor([1] * 6 + [0] * (y.shape[1] - 6))
                .expand(y.shape[0], -1)
                .unsqueeze(-1)
                .bool()
                .to(x.device)
        ...
        temb = timestep_embedding(timesteps, self.model_channels).to(
            self.word_embedding.weight.dtype
        )
        emb = self.time_embed(temb)
