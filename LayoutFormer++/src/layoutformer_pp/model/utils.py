...
class LabelConstraint:
    def __init__(
        self,
        tokenizer: Union[PreTrainedTokenizer, LayoutTransformerTokenizer],
        discrete_degree: int,
        ...
        self.num_token_ids = set(
            self._get_input_ids(" ".join(map(str, range(discrete_degree))))
        )
    ...
    def _get_input_ids(self, in_str: str) -> List[int]:
        if isinstance(self.tokenizer, PreTrainedTokenizer):
            ...
            return self.tokenizer(in_str, add_eos=False, add_bos=False)["input_ids"][
                0
            ].tolist()
    ...
    def prepare(self, label_ids: List[List[int]]):
        ...
        for item_label_ids in label_ids:
            ...
            for lid in item_label_ids.tolist():
                label_name = self.index2label[lid].lower().strip()
                label_count[label_name] += 1
                allow_labels |= self.label_token_ids[label_name]
...
class TransformerSortByDictLabelConstraint:
    def __init__(
        self,
        tokenizer: LayoutTransformerTokenizer,
        discrete_degree: int,
        label_set: Union[List, Set],
        index2label: Dict,
        add_sep_token: bool = False,
...
class TransformerSortByDictRelationConstraint(TransformerSortByDictLabelConstraint):
    def __init__(
        ...
        super().__init__(
            tokenizer, discrete_degree, label_set, index2label, add_sep_token, sep_token
        )
        self.rel_index2type = rel_index2type
        self.discrete_degree = discrete_degree
        self.discrete_fn = DiscretizeBoundingBox(discrete_degree, discrete_degree)

    def prepare(self, label_ids: List[List[int]], relations: List[Tuple]) -> None:
        self.label_constraints, self.relation_constraints, self.decode_state = (
            list(),
            list(),
            list(),
        )
    ...
    def __call__(
        ...
        if state.finished:
            ...
            if (
                ...
                relation_constraints = self.relation_constraints[batch_id][
                    state.curr_element - 1
                ]
                if len(relation_constraints) == 0:
                        ...
                        if state.num_bbox == 0:
                            ...
                            for rel_type, tgt_ele_idx in relation_constraints:
                                ...
                                if rel_type == "left":
                                    ...
                                    plausible_tokens = self._intersect(
                                        plausible_tokens,
                                        set(range(tgt_ele_right, self.discrete_degree)),
                                    )
                            ...
                            for rel_type, tgt_ele_idx in relation_constraints:
                                ...
                                if rel_type == "top":
                                    ...
                                    tgt_ele_bottom = min(
                                        tgt_ele_bbox[1] + tgt_ele_bbox[3] + 3,
                                        self.discrete_degree - 1,
                                    )
                                    plausible_tokens = self._intersect(
                                        plausible_tokens,
                                    ...
                                    plausible_tokens = self._intersect(
                                        plausible_tokens,
                                        set(range(self.discrete_degree // 3 + 1)),
                                    )
                                    ...
                                    plausible_tokens = self._intersect(
                                        plausible_tokens,
                                        set(range(2 * self.discrete_degree // 3)),
                                    )
                                elif rel_type == "being_top":
                                    plausible_tokens = self._intersect(
                                        plausible_tokens,
                                        set(range(5, self.discrete_degree)),
                                    )
                    else:
                        # size
                        if state.num_bbox == 2:
                            ...
                            for rel_type, tgt_ele_idx in relation_constraints:
                                ...
                                if rel_type == "left":
                                    ...
                                    continual_tgt_ele_size = (
                                        self.discrete_fn.continuize_num(
                                            tgt_ele_bbox[2] * tgt_ele_bbox[3]
                                        )
                                    )
                                    continual_tgt_target_size = (
                                        continual_tgt_ele_size * 1.1
                                    )
                                    ...
                                    min_width = max(
                                        0,
                                        _tgt_target_size // (self.discrete_degree - 1),
                                    )
                                    plausible_tokens = self._intersect(
                                        plausible_tokens,
                                        set(range(min_width, self.discrete_degree)),
                                    )
                                elif rel_type == "canvas_equal":
                                    plausible_tokens = self._intersect(
                                        plausible_tokens, {self.discrete_degree - 1}
                                    )
                                elif rel_type == "equal":
                                    continual_tgt_ele_size = (
                                        self.discrete_fn.continuize_num(
                            ...
                            for rel_type, tgt_ele_idx in relation_constraints:
                                ...
                                if rel_type in {"left", "right", "center"}:
                                    ...
                                    if curr_y < tgt_ele_top:
                                        ...
                                        plausible_tokens = self._intersect(
                                            plausible_tokens,
                                            set(range(diff, self.discrete_degree)),
                                        )
                                elif rel_type == "top":
                                    tgt_ele_top = tgt_ele_bbox[1]
                                    ...
                                    plausible_tokens = self._intersect(
                                        plausible_tokens,
                                        set(range(min_height, self.discrete_degree)),
                                    )
                                elif rel_type == "equal":
                                    continual_tgt_ele_size = (
                                        self.discrete_fn.continuize_num(
                                    ...
                                    plausible_tokens = self._intersect(
                                        plausible_tokens,
                                        {height, max(0, height - 1), height + 1},
                                    )
                                elif rel_type == "canvas_equal":
                                    plausible_tokens = self._intersect(
                                        plausible_tokens, {self.discrete_degree - 1}
                                    ...
                                    min_height = min(
                                        2 * (2 * self.discrete_degree // 3 - curr_y),
                                        self.discrete_degree,
                                    )
                                    plausible_tokens = self._intersect(
                                        plausible_tokens,
                                    ...
                                    lower_bound, upper_bound = (
                                        self.discrete_degree // 3,
                                        2 * self.discrete_degree // 3,
                                    )
                                    if curr_y < lower_bound:
                                        min_height = 2 * (lower_bound - curr_y)
                                        max_height = 2 * (upper_bound - curr_y)
                                        plausible_tokens = self._intersect(
                                            plausible_tokens,
                                            set(range(min_height, max_height + 1)),
                                        )
                                    elif lower_bound <= curr_y < upper_bound:
                                        max_height = 2 * (upper_bound - curr_y)
                                        plausible_tokens = self._intersect(
                    ...
                    if len(plausible_tokens) > 0:
                        min_value, max_value = (
                            min(plausible_tokens),
                            max(plausible_tokens),
                        )
                        plausible_tokens |= {
                            i for i in range(min_value - 2, min_value) if i > 0
                        }
                        plausible_tokens |= {
                            i
                            for i in range(max_value + 1, max_value + 3)
                            if i < self.discrete_degree
                        }
