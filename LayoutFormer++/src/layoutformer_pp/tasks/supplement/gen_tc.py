...
class T5LayoutSequenceForGenTC(T5LayoutSequence):
    ...
    def build_type_seq(self, labels) -> str:
        tokens = list()
...
class T5GenTCDataset:
    ...
    def process(self, data) -> Dict:
        ...
        in_type_str = self.seq_processor.build_type_seq(sorted_in_type_labels).lower().strip()
...
def gen_tc_inference(model, data, seq_processor, tokenizer, device, top_k=10, temperature=0.7) -> Tuple[Dict, Dict]:
