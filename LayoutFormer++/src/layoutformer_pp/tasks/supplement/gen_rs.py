...
from layoutformer_pp.tasks.gen_r import RelationTypes, AddCanvasElement, detect_loc_relation, detect_size_relation,  get_label_with_index
...
class T5LayoutSequenceForGenRS(T5LayoutSequence):
...
class T5GenRSDataset:
    def __init__(self, cargs, tokenizer, seq_processor, sort_by_pos: bool = True,
        ...
        if cargs.gen_r_discrete_before_induce_relations:
            ...
            transform_functions.append(AddPositionRelation())
        else:
...
def gen_rs_inference(model, data, seq_processor, tokenizer, device, top_k=10, temperature=0.7, constraint_fn=None):
