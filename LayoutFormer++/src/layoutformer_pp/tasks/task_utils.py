...
from layoutformer_pp.tasks.gen_r import RelationTypes, T5LayoutSequenceForGenR
import layoutformer_pp.tasks.multitask as multitask
from layoutformer_pp.tasks.task_config import TASK_CONFIG
from layoutformer_pp.model import LayoutTransformer
...
def create_fid_model(args, device='cpu'):
    fid_net_dir = Path(__file__).resolve().parents[1] / "net"
    if args.dataset == 'rico':
        fid_net_path = str(fid_net_dir / "fid_rico.pth.tar")
        ...
        fid_net_path = str(fid_net_dir / "fid_publaynet.pth.tar")
