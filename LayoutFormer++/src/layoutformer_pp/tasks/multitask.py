...
from layoutformer_pp.data import RicoDataset, PubLayNetDataset


class T5MultiTaskDataset:
    def __init__(
        self, cargs, tokenizer, task_config, sort_by_pos: bool = True, *args, **kwargs
    ) -> None:
        ...
        for task in self.tasks:
            if task in self.task_config:
                ...
                for param, default_value in self.task_config[task].get(
                    "seq_processor_params", list()
                ):
                    seq_processor_params[param] = getattr(cargs, param, default_value)
                seq_processor = self.task_config[task]["seq_processor"](
                    cargs.dataset,
                    tokenizer,
                    add_sep_token=cargs.add_sep_token,
                    **seq_processor_params,
                )

                task_sort_by_pos = getattr(cargs, f"{task}_sort_by_pos", sort_by_pos)
                dataset = self.task_config[task]["dataset"](
                    cargs,
                    tokenizer,
                    seq_processor,
                    sort_by_pos=task_sort_by_pos,
                    shuffle_before_sort_by_label=getattr(
                        cargs, f"{task}_shuffle_before_sort_by_label", False
                    ),
                    sort_by_pos_before_sort_by_label=getattr(
                        cargs, f"{task}_sort_by_pos_before_sort_by_label", False
                    ),
                )
...
class T5MultiTaskSamplingDataset(T5MultiTaskDataset):
    ...
    def process(self, data) -> Dict:
        ...
        if self.add_task_prompt:
            in_str = "{} {}".format(
                self.task_config[sampled_task]["prompt"], result["in_str"]
            )
        else:
...
class T5MultiTaskRotationDataset(T5MultiTaskDataset):
    def __init__(
        self, cargs, tokenizer, task_config, sort_by_pos, *args, **kwargs
    ) -> None:
        super().__init__(cargs, tokenizer, task_config, sort_by_pos, *args, **kwargs)
    ...
    def __getitem__(self, idx):
        ...
        if self.add_task_prompt:
            in_str = "{} {}".format(
                self.task_config[self.curr_task]["prompt"], result["in_str"]
            )
        else:
            in_str = result["in_str"]
        result.update(
...
class T5MultiTaskPartitionDataset(T5MultiTaskDataset):
    def __init__(
        ...
        task_buckets: str = None,
        permutation_seed: int = 100,
        *args,
        **kwargs,
    ) -> None:
        super().__init__(cargs, tokenizer, task_config, sort_by_pos, *args, **kwargs)
        ...
        if self.num_tasks != len(buckets):
            raise ValueError(
                "Task bucket specification should be equivalent to the number of tasks"
            )
        ...
        # partition
        data_idx = np.random.RandomState(seed=permutation_seed).permutation(
            np.arange(len(self))
        )
        bucket_size = [
            int(np.ceil(len(self) * float(len(tasks) / self.num_tasks)))
...
class T5MultiTaskFineGrainedPartitionDataset(T5MultiTaskDataset):
    def __init__(
        ...
        task_config,
        sort_by_pos: bool = True,
        task_data_size: str = None,
        permutation_seed: int = 100,
        task_weights: str = None,
        *args,
        **kwargs,
    ) -> None:
        super().__init__(cargs, tokenizer, task_config, sort_by_pos, *args, **kwargs)
        ...
        if self.num_tasks != len(self.task_data_size):
            raise ValueError(
                "Task data size specification should be equivalent to the number of tasks"
            )
        if sum(self.task_data_size) > len(self):
            raise ValueError(
                "The sume of task data size shoule not be larger than the size of training data"
            )
...
class T5RicoMultiTaskSamplingDataset(T5MultiTaskSamplingDataset, RicoDataset):
    def __init__(
        ...
        split,
        online_process=True,
        remove_too_long_layout: bool = False,
        sort_by_pos: bool = True,
    ) -> None:
        super().__init__(
            args,
...
class T5RicoMultiTaskFineGrainedPartitionDataset(
    T5MultiTaskFineGrainedPartitionDataset, RicoDataset
):
    def __init__(
        self,
        args,
...
class T5PubLayNetMultiTaskPartitionDataset(
    T5MultiTaskPartitionDataset, PubLayNetDataset
):
    def __init__(
        self,
        args,
...
class T5PubLayNetMultiTaskFineGrainedPartitionDataset(
    T5MultiTaskFineGrainedPartitionDataset, PubLayNetDataset
):
    def __init__(
        self,
        args,
