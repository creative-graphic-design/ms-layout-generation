...
from layoutformer_pp.utils import utils, os_utils, visualization
...
class Generator:
    ...
    def _setup_model(self, ckpt_path: str, ds_ckpt_tag: str = None):
        if self.args.trainer == "ddp":
            ...
            self.model, _, _, _ = deepspeed.initialize(
                args=self.args, model=self.model, model_parameters=params
            )
        else:
            self.model.to(self.device)
        ...
        if ckpt_path:
            if self.args.trainer == "deepspeed":
                ...
                state_dict = torch.load(ckpt_path, map_location=self.device)
                state = OD(
                    [(key.split("module.")[-1], state_dict[key]) for key in state_dict]
                )
    ...
    def default_d2c_fn(self, bbox):
        discrete_fn = DiscretizeBoundingBox(
            num_x_grid=self.args.discrete_x_grid, num_y_grid=self.args.discrete_y_grid
        )
    ...
    def aggregate_metrics(self, metric, layouts, gold_mask, pred_mask):
        ...
        self.fid.collect_features(
            layouts["pred_bboxes"].to(self.device),
            layouts["pred_labels"].to(self.device),
            pred_padding_mask.to(self.device),
        )
        self.fid.collect_features(
            layouts["gold_bboxes"].to(self.device),
            layouts["gold_labels"].to(self.device),
            gold_padding_mask.to(self.device),
            real=True,
        )

        self.alignment += metrics.compute_alignment(
            layouts["pred_bboxes"], pred_mask
        ).tolist()
        ...
        if self.args.dataset == "rico":
            self.new_overlap += metrics.compute_overlap_ignore_bg(
                layouts["pred_bboxes"], layouts["pred_labels"], pred_mask
            ).tolist()
    ...
    def switch_task(self, task: str, saved_layouts: List[str]):
        ...
        self.metrics_out_dir = (
            Path(self.args.out_dir) / task / self.ds_ckpt_tag / "metrics.pkl"
        )
        self.results_out_dir = (
            Path(self.args.out_dir) / task / self.ds_ckpt_tag / "results.pkl"
        )
        if self._is_main_process:
            os_utils.makedirs(self.image_out_dir)
        ...
        if self._is_main_process:
            ...
            with torch.no_grad():
                ...
                for batch_idx, data in enumerate(self.test_dataloader):
                    ...
                    gold_mask = (
                        out["gold_mask"] if "gold_mask" in out else out["mask"]
                    ).cpu()
                    pred_mask = (
                        out["pred_mask"] if "pred_mask" in out else out["mask"]
                    ).cpu()
                    ...
                    if "input" in self.saved_layouts:
                        ...
                        input_mask = (
                            out["input_mask"] if "input_mask" in out else out["mask"]
                        ).cpu()
                    ...
                    self.aggregate_metrics(metrics, layouts, gold_mask, pred_mask)
