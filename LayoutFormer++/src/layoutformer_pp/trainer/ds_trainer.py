...
class DSTrainer(Trainer):
    ...
    def _setup_experiment(self, task_name: str, task_config: Dict):
        ...
        print(
            f"World size: {self._world_size}, Local rank: {self._local_rank}, Main Process: {self._is_main_process}"
        )
        ...
        if self._is_main_process:
            import layoutformer_pp.trainer.basic_trainer as basic_trainer

            # Keep wandb consistent with this module (supports test patching).
            basic_trainer.wandb = wandb
            task_config.update(
                {
                    "warmup_steps": self._ds_config["scheduler"]["params"][
                        "warmup_num_steps"
                    ]
                }
            )
            super()._setup_experiment(task_name, task_config)
            dst = os.path.join(self.args.out_dir, "ds_config.json")
            if os.path.abspath(self.args.deepscale_config) != os.path.abspath(dst):
                shutil.copy(self.args.deepscale_config, dst)
    ...
    def _load_ckpt(self):
        _, client_state = self.model_engine.load_checkpoint(
            self.train_ckpt_path,
            load_optimizer_states=False,
            load_lr_scheduler_states=False,
            load_module_only=True,
            tag=self.args.ds_ckpt_tag,
        )  # model engine
        print(client_state)
    ...
    def _setup_dataloader(self):
        ...
        assert (
            self._ds_config["gradient_accumulation_steps"]
    ...
    def __call__(self, train_step, eval_step, eval_interval=1):
        ...
        for epoch in range(self.args.epoch):
            ...
            for batch_idx, data in enumerate(self.train_dataloader):
                ...
                if (
                    global_step % self.args.train_log_step == 0
            ...
            self.model_engine.save_checkpoint(
                save_dir=self._normal_ckpt_path, client_state={"epoch": epoch + 1}
            )

            if self._is_main_process and (
                epoch == (self.args.epoch - 1) or (epoch + 1) % eval_interval == 0
            ):
                ...
                with torch.no_grad():
                    for data in self.val_dataloader:
                        ...
                        pred_labels = (
                            out["gold_labels"]
                            if self.is_label_condition
                            else out["pred_labels"]
                        )
                        mask = out["mask"]

                        pred_layouts.extend(
                            self.collect_layouts(pred_bboxes, pred_labels, mask)
                        )

                        if epoch == 0:
                            gold_labels, gold_bboxes = (
                                out["gold_labels"],
                                out["gold_bboxes"],
                            )
                            gold_layouts.extend(
                                self.collect_layouts(gold_bboxes, gold_labels, mask)
                            )

                self.logging_metrics(epoch, global_step)

                if self.is_debug:
                    self.save_val_output(
                        gold_layouts, pred_layouts, data["out_str"], out["out_str"]
                    )
                ...
                wandb.log(
                    {
                        f"checkpoint_measure-{self.ckpt_measurement.measurement}": measure
                    },
                    step=global_step,
                )
                print(
    ...
    def do_checkpointing(self, epoch, is_best):
        if is_best:
            if not os.path.exists(self._normal_ckpt_path):
                return
        ...
        if epoch != self.args.epoch - 1 and os.path.exists(self._normal_ckpt_path):
